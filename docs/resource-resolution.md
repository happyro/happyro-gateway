...
2. `DATA_OVERRIDE_PATH` 中按顺序的目录。当前为 `inputs/runtime/kro-20211105/client`、其下的 `data/`，然后是 `localization/client/data`。覆盖根目录拼接时会去掉请求路径里的 `data/` 前缀，并将客户端 CP949 字节形式的韩文路径还原为 Unicode。
